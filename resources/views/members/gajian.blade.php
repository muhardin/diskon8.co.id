                        ...
                        <div class="row">
                            <div class="col-sm-4">
                                <a class="element-box el-tablo" href="#">
                                    ...
                                    <div class="value">{{"Rp. ".number_format(@$collectionoutstanding->where('status',0)->sum('balanced'),0)}}</div>
                                </a>
                            </div>
                            <div class="col-sm-4">
                                <a class="element-box el-tablo" href="#">
                                    <div class="label"> Saldo Tersedia </div>
                                    <div class="value">{{"Rp. ".number_format(@$collectionsaldo->sum('balanced'),0)}}</div>
                                </a>
                            </div>
                            <div class="col-sm-4">
                                <a class="element-box el-tablo" href="#">
                                    ...
                                    <div class="value">
                                            {{"Rp. ".number_format(@$collectionoutstanding->where('status','==','1')->sum('balanced'),0)}}
                                    </div>
                                </a>
                            </div>
                        </div>
                        ...
                        <div class="os-tabs-w p-0"> 
                            <div class="os-tabs-controls mb-4">
                                <ul class="nav nav-tabs upper">
                                    <li class="nav-item">
                                    <a aria-expanded="false" class="nav-link {{@$mutasiact}}"
                                            href="{{url('/member/gajian')}}">Mutasi</a>
                                    </li>
                                    <li class="nav-item">
                                        <a aria-expanded="false" class="nav-link {{@$requestact}}"
                                            href="{{url('/member/gajian?request=yes')}}">Request Pencairan</a>
                                    </li>
                                </ul>
                            </div>
                        </div>
                            ...
                            <div class="row">
                                ...
                                <div class="col-sm-6">
                                    <div class="element-actions">
                                        <a class="btn btn-primary trigger-button" href="#">Tarik Profit</a>
                                        <div id="dimmer"></div>
                                    </div>
                                </div>
                            </div>
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-6">
                                <div class="element-box popup invisible-box">
                                    <div class="header">
                                        <h5>Saldo Tersedia: {{"Rp. ".number_format(@$collectionsaldo->sum('balanced'),0)}}</h5>
                                    ...
                                    <form class="m-2" id="form-payout" novalidate="true">
                                        ...
                                        <div class="form-group kode-sms" style="display:none">
                                            <label for="formGroupExampleInput">Kode SMS</label>
                                            <input type="text" class="form-control" id="kodesms" name="kodesms"
                                                placeholder="Masukkan Kode SMS"><br>
                                                <button class="btn btn-outline-primary mx-2 pull-right sms-button"
                                                type="button">Kirim SMS</button>
                                        </div>
                            ...
                            <table class="table table-bordered table-lg table-v2 table-striped">
                                ...
                                <tbody>
                                    ...
                                    <tr>
                                        ...
                                        <td class="text-center">
                                        {{"Rp. ".(number_format(abs(@$item->balanced),0))}}
                                        </td>
                                        <td class="text-center">
                                            @if(@$item->status==0)
                                            <button class="btn btn-outline-primary mx-2 pull-right"
                                            ...
                                            @else
                                            <button class="btn btn-outline-success mx-2 pull-right"
                            ...
                            <table class="table table-bordered table-lg table-v2 table-striped">
                                ...
                                <tbody>
                                    ...
                                    <tr>
                                        ...
                                        <td class="text-center">
                                            <a href="{{url('member/order/detail/'.@$item->order_id)}}">{{"Rp. ".(number_format(abs(@$item->balanced),0))}}</a>
                                        </td>
                                    </tr>
                                    @endforeach
                                </tbody>
...
<script type = "text/javascript" >
    $(document).ready(function() {
        ...
        $('#nominal').keyup(_.debounce(function(e) {
            ...
            } else if (nominal > {{$collectionsaldo->sum('balanced')}}) {
                return swal('Oops!', 'Nominal melebihi saldo yang ada', 'info');
            }
            axios.post('{{url('/api/gajian')}}', {
                nominal: nominal,
                bank_id: 101,
        ...
        $('#form-payout').validator().submit(function(e) {
            ...
            data.smscode = Number($('#kodesms').unmask());
            data.bank_id = 101;
            data.user_id = {{Auth::user()->id}};
            if (data.nominal < 100000) {
                return swal('Oops!', 'Nominal minimum untuk payout Rp 100.000', 'info');
            } else if (data.nominal > {{$collectionsaldo->sum('balanced')}}) {
                return swal('Oops!', 'Nominal melebihi saldo yang ada', 'info');
            }
            $('.overlay').show();
            axios.post('{{url('/api/gajianpost')}}', data, {
                headers: {
                    'Authorization': 'Bearer eyJ0eXAiOiJKV1QiLCJhbGciOiJSUzI1NiIsImp0aSI6ImI5ZDI3YTlmZTc3ZjMxOGM1YmQ1M2U5YzJiZWI0NDU3NDBkYWM2MTY0MWNmODg2ZjdkNmM5ZGZhNmI2MWRlMTI3MWIzYjQ5ZWVlODA5YzFiIn0.eyJhdWQiOiI1IiwianRpIjoiYjlkMjdhOWZlNzdmMzE4YzViZDUzZTljMmJlYjQ0NTc0MGRhYzYxNjQxY2Y4ODZmN2Q2YzlkZmE2YjYxZGUxMjcxYjNiNDllZWU4MDljMWIiLCJpYXQiOjE1NjA1ODIwNjUsIm5iZiI6MTU2MDU4MjA2NSwiZXhwIjoxNTkyMjA0NDY1LCJzdWIiOiIyMDQ3Iiwic2NvcGVzIjpbXX0.R1i42NAnC04tn2Z6nsx7UC6ipyBSKphe9e9lgg3lmBlsLQFjrFr7uJ6i3EEaYbfjw_corK2JgJz4E9_yT_RxsYNveINIeVlnBnKUlo0LsrwaVtbDXGsraCpJD7Qj5lFpkD23wuxd6WawtzB-YFiH01ikeD3l3MEey7H-RiP7CnDO9eSwXdNLUNX2_yuBvlrQSZeyBWAxjBDdaiLi-5ukHCVE_bifMbl43w5qeEwNqzHxaHjPBmyi1KZE2-1IzDA4PDo29owrmblMDu13p6NDS6i2EWsfdB7pNpt8ypJ4CxfVgf6xmg7Z2m2fO1iy-bRN1k4wx9NXv2NhiqlUsvlQjAbeaNl0V-g2koPbwVyrcOE9Yd5nnfG-JBsrAtdythDB-Y90v7InijP6yxF_gUwSKsMs_kj35V88YmP7fFTQDXgBPCw2nBWqlxrNYvwcc72tMzGDXhzhsbiY-y1ZWikSGt9ooypW557snmYUl0-DmDUG96YVToAc89Xrnpuf1Amdq4oNjvviHt5-ezpq_1bCLu_l8fZbSVXfXCFcqjI4zqLt0COxqXHQahuwOM6kRCS5Je7j1EDtqzY93U4mS-HM70l5mbPDLMdpY-Iah9XahsBoTOFmYVtkpem1RJtQcSIGbjnQCBivFSNq9fJ76Zl9LFybsgk96ZV32tSX7_HDVe8'
                }
            }).then(function(response) {
                ...
                if (response.data.meta.msg=='Berhasil') {
                    swal(response.data.meta.msg, response.data.meta.message, response.data.meta.msgbox).then(function(params) {

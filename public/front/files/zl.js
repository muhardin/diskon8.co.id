...
app={wTop:document.documentElement.scrollTop||document.body.scrollTop,slideshows:[],goodsId:0,nowProductData:{}};
...
function getGoodsId(){
	var get=window.location.href.substring(window.location.href.lastIndexOf("/F")+2,window.location.href.length);
...
function openTime(startTime,sumTime){
	...
	time.s=startTime%60;time.m=Math.floor(startTime/60)%85;time.h=Math.floor(startTime/60/60);
...
function addJs(){setTimeout(function(){var add_js="<script src='js/data_add.js'><\/script><script src='js/product_2.js'><\/script>";$("body").append(add_js);},300);}
